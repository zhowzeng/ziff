...
2. Additional screens — sync/connection states. **Out of scope for v1 (2026-08-23):** a PR list/inbox screen — v1 works against local repos/diffs only, no GitHub PR sync list. **Resolved (2026-08-23):** "copy context to agent" isn't a screen/flow — it's just copy-to-clipboard, already implemented (`ContextDrawer`'s "Copy all for agent" button + per-item copy in `ContextItem`, plus a `⌘⇧C`/`Ctrl+Shift+C` shortcut). **Resolved (2026-08-23):** settings now has a real screen (`SettingsModal`, see "UI kits") and empty states are designed and ported (`EmptyState`, no-project / no-diff / no-match) — the old `Settings` icon button on `src/routes/+page.svelte` (the tokens demo home page, not the review screen) is still unwired, but that page was never meant to host it; the real entry point is the sidebar's "Settings" row in `review/+page.svelte`. Still open: sync/connection states (e.g. what the app shows while fetching, or when GitHub is unreachable) haven't been designed yet.
