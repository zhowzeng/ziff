...
**Svelte-only additions, no React/claude.ai/design source (2026-08-23)** — common primitives the desktop-app screen didn't need yet but future screens (settings, sync states) will: `Modal` (backdrop + card, Escape/backdrop-click to close), `Toast` (+ `src/lib/stores/toast.svelte.js`'s `toast()`/`dismiss()` — mounted once in `+layout.svelte`, call `toast('message', { variant, duration })` from anywhere), `Checkbox`, `Switch`, `Tooltip`. Built by hand against the same token set and inline-style convention as the ported components, not against a spec — treat as a first pass, open to adjustment once real settings/sync screens are designed.
