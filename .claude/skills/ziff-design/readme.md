...
**Resolved as of 2026-08-23 (kept for history, not action items):** additional screens — PR list/inbox (ruled out of scope for v1: local repos/diffs only), settings, empty states, the copy-to-agent flow (turned out to be plain copy-to-clipboard), and the Project dropdown's "add new" entry are all designed and shipped in `src/lib/components/` and `review/+page.svelte`. The original "need the real Ziff codebase" question is moot — this repo *is* that codebase now, built directly against this design system rather than waiting on it.
