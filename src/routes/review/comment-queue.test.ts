...
describe('itemsFor', () => {
  it('returns only the asking Repo, leaving the other Repo untouched', () => {
    queue.add({ repoId: 'a', file: 'Cargo.toml', text: 'from a', ...anchor(12) });
    queue.add({ repoId: 'b', file: 'Cargo.toml', text: 'from b', ...anchor(12) });
...
describe('find', () => {
  ...
  it('treats an absent lineEnd as a single-line range', () => {
    queue.add({ repoId: 'a', file: 'Cargo.toml', text: 'one line', ...anchor(12) });
  ...
  it('distinguishes the two numbering spaces', () => {
    queue.add({ repoId: 'a', file: 'Cargo.toml', text: 'diff side', ...anchor(12) });
...
describe('lineKeys', () => {
  it('does not mark lines a comment in another Repo covers', () => {
    queue.add({ repoId: 'a', file: 'Cargo.toml', text: 'from a', ...anchor(12, 14) });
    queue.add({ repoId: 'b', file: 'Cargo.toml', text: 'from b', ...anchor(30) });
